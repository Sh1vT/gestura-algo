...
class _CameraStreamAppState extends State<CameraStreamApp> {
  ...
  Future<void> _getLocalIp() async {
    ...
    if (wifiIP != null) {
      ...
      _ipController.text = "192.168.244.169";
      setState(() {});
    }
  }
  ...
  void _initWebSocket() {
    ...
    _channel?.sink.close();
    _channel = WebSocketChannel.connect(Uri.parse("ws://$ip:8000/ws"));
        ...
        _speech.listen(
          onResult: (val) {
            setState(() {
              ...
              _detectedWords.add(val.recognizedWords);
              String sentence = _detectedWords.join(' ');
              _detectedSign = sentence[0].toUpperCase() + sentence.substring(1);
              _isListening = false;
            });
          },
        );
  ...
  void _translate() async {
    final ip = _ipController.text;
    final uri = Uri.parse("http://$ip:8000/nlp_process");
          ...
          child: Row(
            ...
            children: [
              Column(
                ...
                children: [
                  // Row(children: [TitleWidget(), SizedBox(width: 180,height: 40, child: TextField(controller: _ipController,))]),
                  TitleWidget(),
